...
import { TypeProvider, t } from "../src/providers/TypeProvider.ts";
...
describe("TypeProvider", () => {
  ...
  describe("Codec Types", () => {
    describe("bigint", () => {
      it("should decode from string", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.bigint();

        expect(
          alepha.codec.decode(schema, "123456789012345678901234567890"),
        ).toBe(BigInt("123456789012345678901234567890"));
        expect(alepha.codec.decode(schema, "0")).toBe(BigInt(0));
        expect(alepha.codec.decode(schema, "-999")).toBe(BigInt(-999));
      });

      it("should decode from number", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.bigint();

        expect(alepha.codec.decode(schema, 12345)).toBe(BigInt(12345));
      });

      it("should decode from bigint", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.bigint();

        expect(alepha.codec.decode(schema, BigInt(999))).toBe(BigInt(999));
      });

      it("should encode to string", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.bigint();

        expect(alepha.codec.encode(schema, BigInt(123))).toBe("123");
        expect(alepha.codec.encode(schema, BigInt(-456))).toBe("-456");
      });
      ...
      it("should reject invalid values", async () => {
        ...
        expect(() => alepha.codec.decode(schema, "not a number")).toThrow();
        expect(() => alepha.codec.decode(schema, "12.34")).toThrow();
      ...
      it("should round-trip encode/decode", async () => {
        ...
        expect(encoded).toBe("123456789012345678901234567890");
    ...
    describe("url", () => {
      it("should decode valid URLs", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.url();

        const url1 = alepha.codec.decode(schema, "https://example.com");
        expect(url1).toBeInstanceOf(URL);
        expect(url1.href).toBe("https://example.com/");

        const url2 = alepha.codec.decode(
          schema,
          "https://example.com/path?query=value#hash",
        );
        expect(url2).toBeInstanceOf(URL);
        expect(url2.pathname).toBe("/path");
        expect(url2.search).toBe("?query=value");
        expect(url2.hash).toBe("#hash");
      });

      it("should encode URLs to string", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.url();

        const encoded = alepha.codec.encode(
          schema,
          new URL("https://example.com/test"),
        );
        expect(encoded).toBe("https://example.com/test");
      });
      ...
      it("should reject invalid URLs", async () => {
        ...
        expect(() => alepha.codec.decode(schema, "not a url")).toThrow();
    ...
    describe("binary", () => {
      it("should encode binary data to base64", async () => {
        const alepha = Alepha.create();
        await alepha.start();
      ...
      it("should decode base64 to Uint8Array", async () => {
        const alepha = Alepha.create();
        await alepha.start();
        const schema = t.binary({});

        const base64 = btoa("Test");
        const result = alepha.codec.decode(schema, base64);
