...
describe("Relations", () => {
	...
	// test("nested relations - user -> posts -> comments", async () => {
	// 	class App {
	// 		users = $repository(users, {
	// 			posts: {
	// 				type: "many",
	...
	// 	await alepha.start();
	//
	// 	const user = await app.users.create({ name: "Charlie Brown" });
	// 	const post = await app.posts.create({
	// 		authorId: user.id,
	// 		title: "Nested Relations",
	// 		content: "Testing nested loading",
	// 	});
	// 	const comment = await app.comments.create({
	// 		postId: post.id,
	// 		authorId: user.id,
	// 		text: "Great post!",
	// 	});
	...
	// 	const userWithNested = await app.users.findOne({
	// 		where: { id: user.id },
	// 		with: {
	// 			posts: {
	// 				with: { comments: true },
