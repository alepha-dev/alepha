...
import { AlephaFile } from "../src/index.ts";
import { NodeFileSystemProvider } from "../src/providers/NodeFileSystemProvider.ts";
...
describe("NodeFileSystem", () => {
  ...
  describe("getContentType", () => {
    it("should get content type from filename", ({ expect }) => {
      expect(fs.getContentType("test.json")).toBe("application/json");
      expect(fs.getContentType("image.png")).toBe("image/png");
      expect(fs.getContentType("document.pdf")).toBe("application/pdf");
    });

    it("should return default for unknown extensions", ({ expect }) => {
      expect(fs.getContentType("file.unknown")).toBe(
  ...
  describe("detectFileType", () => {
    it("should detect PNG from stream", async ({ expect }) => {
      ...
      const stream = Readable.from(pngMagic);

      const result = await fs.detectFileType(stream, "test.png");
    ...
    it("should detect JPEG from stream", async ({ expect }) => {
      ...
      const stream = Readable.from(jpegMagic);

      const result = await fs.detectFileType(stream, "photo.jpg");
    ...
    it("should fall back to extension when magic bytes don't match", async ({
      expect,
    }) => {
      const textContent = Buffer.from("Hello, world!");
      const stream = Readable.from(textContent);

      const result = await fs.detectFileType(stream, "file.txt");
  ...
  describe("integration: create and detect", () => {
    it("should create file from text and detect type", async ({ expect }) => {
      const file = fs.createFile({
        text: "Hello, world!",
        name: "test.txt",
      });
      ...
      // Get stream and detect
      const stream = file.stream();
      const detection = await fs.detectFileType(
        stream instanceof Readable ? stream : Readable.from(stream),
        file.name,
      );

      expect(detection.mimeType).toBe("text/plain");
      expect(detection.verified).toBe(false); // Text files don't have magic bytes
    });

    it("should create file from buffer with PNG magic bytes", async ({
      expect,
    }) => {
      const pngMagic = Buffer.from([
        0x89,
      ...
      expect(detection.mimeType).toBe("image/png");
      expect(detection.verified).toBe(true);
    });

    it("should detect mismatch between extension and actual content", async ({
      expect,
    }) => {
      // Create a PNG but name it .txt
      const pngMagic = Buffer.from([
        0x89,
        0x50,
        0x4e,
      ...
      expect(file.name).toBe("fake.txt");
      expect(file.type).toBe("text/plain");

      // But detection reveals it's actually a PNG
      const stream = file.stream();
      const detection = await fs.detectFileType(
        stream instanceof Readable ? stream : Readable.from(stream),
        file.name,
