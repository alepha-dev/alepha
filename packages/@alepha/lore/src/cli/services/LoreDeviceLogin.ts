...
import { type LoreToken, LoreTokenStore } from "./LoreTokenStore.ts";

/**
 * The OAuth 2.0 device flow (RFC 8628) against a Lore instance, so a laptop
 * can talk to Lore without pasting a token into a shell: the body of
 * `lore login` and `lore logout`, and of the platform adapter's own.
 *
 ...
 * flow would poll until it timed out. Two things prevent it, and neither is a
 * convention:
 *
 ...
 * - {@link login} refuses to run in CI outright.
 */
export class LoreDeviceLogin {
  ...
  /**
   * Sign this machine in to the configured Lore instance, and store the token.
   */
  public async login(): Promise<void> {
    ...
    // `cli`, a scope of its own. The approval page shows the human the scope
    // beside the instance's copy for it, and this used to borrow `mcp` - the
    // one Lore made for Claude connecting - so a sign-in from a terminal read
    // as an MCP connection (#Q2244). Nothing enforces scopes, so the name
    // costs nothing but what the page says.
  ...
  /**
   * Forget this machine's login for the configured instance.
   *
   * "Nothing to forget" is said out loud rather than reported as a success: a
   * logout that quietly does nothing leaves someone believing they revoked a
   * credential they are still carrying, on some other hostname.
   */
  ...
  protected async poll(
    ...
    let interval = Math.max(
      start.interval ?? LoreDeviceLogin.DEFAULT_INTERVAL_SECONDS,
      LoreDeviceLogin.MIN_INTERVAL_SECONDS,
    );
    const deadline =
      this.dateTime.nowMillis() +
      (start.expires_in ?? LoreDeviceLogin.DEFAULT_EXPIRY_SECONDS) * 1000;
