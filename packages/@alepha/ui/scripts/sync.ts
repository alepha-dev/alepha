...
const KEEP_LOCAL = new Map([
  [
    "ui/button.tsx",
    "the `loading` prop (spinner overlay, aria-busy, disabled-while-busy, which every submit in the kit relies on) and the disabled cursor block, whose comment carries the reason the hover utilities must stay unprefixed",
  ],
  [
    "ui/badge.tsx",
