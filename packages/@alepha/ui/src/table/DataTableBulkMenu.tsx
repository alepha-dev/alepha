...
export interface DataTableBulkMenuProps<T> {
 ...
 */
export const DataTableBulkMenu = <T,>(props: DataTableBulkMenuProps<T>) => {
  const { tr } = useI18n();
      ...
      <DropdownMenuContent side="top" align="center">
        {resolved?.failed ? (
          ...
          <DropdownMenuItem disabled>
            ...
            {tr("dataTable.bulkMenu.loading", { default: "Loading…" })}
          </DropdownMenuItem>
        ) : resolved.items.length === 0 ? (
          <DropdownMenuItem disabled>
            {tr("dataTable.bulkMenu.empty", { default: "Nothing to pick" })}
          </DropdownMenuItem>
        ) : (
