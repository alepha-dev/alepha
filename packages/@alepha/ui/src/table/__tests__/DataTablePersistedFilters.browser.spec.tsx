 ...
 * Activity).
 */
describe("DataTable (persisted filters that changed shape)", () => {
  ...
  it("wraps a stored scalar for a filter that is now an array", async () => {
    ...
    await mount(
      <DataTable<Row, typeof arrayFields>
  ...
  it("takes the first element when an array meets a filter that is now scalar", async () => {
    ...
    const seen: Array<Record<string, any> | undefined> = [];
    await mount(
      <DataTable<Row, typeof scalarFields>
        persistenceKey="probe"
        columns={columns}
  ...
  it("restores a stored operator under the name it was stored with", async () => {
    ...
    const seen: Array<Record<string, any> | undefined> = [];
    await mount(
      <DataTable<Row, typeof operatorFields>
        persistenceKey="probe"
        columns={columns}
