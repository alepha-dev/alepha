...
/**
 * `AlephaTable`'s filters, typed from `fields`.
 *
 ...
 */
describe("AlephaTable filter fields", () => {
  describe("types", () => {
  ...
  describe("alephaTableFilterKeys", () => {
    it("lists each field, followed by its operator key, in declaration order", () => {
      expect(alephaTableFilterKeys(filterFields)).toEqual([
        "search",
        "status",
        "statusOp",
        "tags",
  ...
  describe("alephaTableFilterMode", () => {
    it("locks the search preset, and makes every other field optional unless it says otherwise", () => {
      expect(alephaTableFilterMode(filterFields.search)).toBe("locked");
      expect(alephaTableFilterMode(filterFields.status)).toBe("optional");
      expect(alephaTableFilterMode(filterFields.owner)).toBe("default");
      expect(alephaTableFilterMode({ preset: "search", mode: "default" })).toBe(
        "default",
      );
    });
  });

  describe("buildAlephaTableFilterSchema", () => {
    const schema = buildAlephaTableFilterSchema(filterFields);

    it("declares every key, operator keys included", () => {
      expect(Object.keys(z.schema.shape(schema))).toEqual(
        alephaTableFilterKeys(filterFields),
      );
    });
    ...
    it("takes a custom operator list's values", () => {
      const custom = buildAlephaTableFilterSchema({
        size: {
          schema: z.string(),
          operators: [
