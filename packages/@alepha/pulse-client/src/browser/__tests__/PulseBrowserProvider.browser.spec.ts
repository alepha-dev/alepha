...
import { PulseBrowserProvider } from "../PulseBrowserProvider.ts";

describe("PulseBrowserProvider", () => {
  it("enqueues a pageview on react:transition:end (prod + browser)", async () => {
    ...
    const provider = alepha.inject(PulseBrowserProvider);
    await alepha.start();
  ...
  it("does not enqueue a pageview when the beacon feature is off", async () => {
    ...
    // and discarded later.
    alepha.store.set(pulseClientAtom, {
