...
import { sigilFingerprintSource } from "../sigilFingerprint.ts";

describe("sigilFingerprintSource", () => {
  it("keeps one identity across a rebuild", () => {
    ...
    const before = sigilFingerprintSource(
      "TypeError",
      "TypeError: x\n    at render (https://app/entry.iHryQ0pA.js:12:44)",
    );
    const after = sigilFingerprintSource(
      "TypeError",
  ...
  it("separates different throw sites", () => {
    const a = sigilFingerprintSource(
      "TypeError",
      "TypeError: x\n    at render (app.js:1:1)",
    );
    const b = sigilFingerprintSource(
      "TypeError",
      "TypeError: x\n    at save (app.js:1:1)",
    );
  ...
  it("separates different error names at the same site", () => {
    ...
    expect(sigilFingerprintSource("TypeError", stack)).not.toBe(
      sigilFingerprintSource("RangeError", stack),
    );
  });
  ...
  it("uses the first frame, not the message line", () => {
    ...
    const withId = sigilFingerprintSource(
      "Error",
      "Error: user 4711 not found\n    at load (app.js:3:9)",
    );
    const withOther = sigilFingerprintSource(
      "Error",
  ...
  it("still fingerprints a stack with no frames", () => {
    ...
    const a = sigilFingerprintSource("Error", "Script error.");
    const b = sigilFingerprintSource("Error", "Out of memory");
  ...
  it("carries no app identifier, so a fingerprint survives moving sinks", () => {
    const fp = sigilFingerprintSource(
      "Error",
      "Error: x\n    at f (app.js:1:1)",
    );
