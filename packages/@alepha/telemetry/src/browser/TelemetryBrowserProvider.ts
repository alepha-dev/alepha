...
import type { TelemetryTracker } from "../shared/telemetryFeatures.ts";
import { TelemetryQueue } from "./TelemetryQueue.ts";
import { TelemetryVitals } from "./TelemetryVitals.ts";

/**
 * Browser bootstrap: subscribes to framework hooks, batches telemetry, and
 * posts the envelope to the same-origin `/api/telemetry/ingest` proxy. No
 * credential lives here. Active in production + browser only.
 *
 * Each tracker is gated by {@link telemetryClientAtom} — the SSR-hydrated view
 ...
 */
export class TelemetryBrowserProvider {
  ...
  protected readonly start = $hook({
    ...
    handler: () => {
      ...
      this.queue = new TelemetryQueue(send as any);
   ...
   */
  protected wants(tracker: TelemetryTracker): boolean {
    const config = this.alepha.store.get(telemetryClientAtom);
