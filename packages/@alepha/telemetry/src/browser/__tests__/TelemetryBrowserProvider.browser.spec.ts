...
import { TelemetryBrowserProvider } from "../TelemetryBrowserProvider.ts";

describe("TelemetryBrowserProvider", () => {
  it("enqueues a pageview on react:transition:end (prod + browser)", async () => {
    ...
    const provider = alepha.inject(TelemetryBrowserProvider);
    await alepha.start();
  ...
  it("does not enqueue a pageview when the beacon feature is off", async () => {
    ...
    // and discarded later.
    alepha.store.set(telemetryClientAtom, {
