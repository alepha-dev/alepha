...
} from "../shared/schemas/telemetryConfig.ts";
import type { TelemetryEnvelope } from "../shared/schemas/telemetryEnvelope.ts";
import { telemetryClientAtom } from "../shared/telemetryClientAtom.ts";
import {
  allTrackersEnabled,
  type TelemetryTracker,
} from "../shared/telemetryFeatures.ts";
import { telemetryFingerprintSource } from "../shared/telemetryFingerprint.ts";
 ...
 *
 * **Works with no sink at all.** Without `TELEMETRY_SINK` / `TELEMETRY_KEY`
...
export class TelemetrySinkProvider {
  ...
  protected env = $env(telemetryEnv);

  /** Errors waiting to be sent, keyed by fingerprint source. */
  protected readonly pendingErrors = new Map<string, AggregatedError>();
  protected pendingViews: NonNullable<TelemetryEnvelope["views"]> = [];
  protected pendingVitals: NonNullable<TelemetryEnvelope["vitals"]> = [];
  protected pendingMetrics: NonNullable<TelemetryEnvelope["metrics"]> = [];
  protected pendingHeartbeat?: TelemetryEnvelope["heartbeat"];
  ...
  protected readonly init = $hook({
    ...
    handler: () => {
      ...
      if (!this.hasSink()) {
        this.log.info(
          "Telemetry has no sink (TELEMETRY_SINK / TELEMETRY_KEY unset) — capturing locally, sending nothing.",
        );
      }
      ...
      this.alepha.events.on("react:server:render:begin", () => {
        this.alepha.store.set(telemetryClientAtom, {
          enabled: this.enabledTrackers(),
          sampling: this.config.sampling ?? TELEMETRY_CONFIG_DEFAULTS.sampling,
          excludedPaths:
            this.alepha.store.get(telemetryOptions).excludedPaths ?? [],
  ...
  public hasSink(): boolean {
    return !!(this.env.TELEMETRY_SINK && this.env.TELEMETRY_KEY);
  }

  public sinkOrigin(): string {
    return (this.env.TELEMETRY_SINK ?? "").replace(/\/$/, "");
  }
   ...
   * to stop observing.
   */
  public enabledTrackers(): Record<TelemetryTracker, boolean> {
    ...
    this.configInFlight = (async () => {
      try {
        const res = await this.http.fetch(
          ...
          {
            method: "GET",
            headers: { authorization: `Bearer ${this.env.TELEMETRY_KEY}` },
          },
        );
        this.config = (res.data ?? {}) as TelemetryConfig;
   ...
   */
  protected aggregate(error: NonNullable<TelemetryEnvelope["errors"]>[number]) {
    const key = telemetryFingerprintSource(error.name, error.stack);
    ...
    try {
      await this.http.fetch(`${this.sinkOrigin()}/api/ingest`, {
        ...
        headers: {
          ...
          authorization: `Bearer ${this.env.TELEMETRY_KEY}`,
        },
   ...
   */
  protected report(envelope: TelemetryEnvelope): void {
    for (const error of envelope.errors ?? []) {
      this.log.warn(
