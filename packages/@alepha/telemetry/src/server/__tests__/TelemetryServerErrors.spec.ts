...
import { TelemetryServerErrors } from "../TelemetryServerErrors.ts";
import { TelemetrySinkProvider } from "../TelemetrySinkProvider.ts";

class FakeSink extends TelemetrySinkProvider {
  public ingested: any[] = [];
...
const make = () =>
  ...
  }).with({ provide: TelemetrySinkProvider, use: FakeSink });
...
describe("TelemetryServerErrors", () => {
  it("tags a server crash as server-origin", async () => {
    const alepha = make();
    alepha.inject(TelemetryServerErrors);
    await alepha.start();
    ...
    const sink = alepha.inject(TelemetrySinkProvider) as FakeSink;
  ...
  it("ignores 401 and 403, which are outcomes rather than crashes", async () => {
    ...
    expect(
      (alepha.inject(TelemetrySinkProvider) as FakeSink).ingested,
    ).toHaveLength(0);
  });
