...
import { TelemetryProxyController } from "../TelemetryProxyController.ts";
...
describe("TelemetryProxyController.ingest", () => {
  it("stamps country + a stable visitor hash from the request edge and forwards", async () => {
    const alepha = make();
    const ctrl = alepha.inject(TelemetryProxyController);
    await alepha.start();
    ...
    const fwd = alepha.inject(TelemetrySinkProvider) as FakeSink;
