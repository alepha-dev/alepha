...
import { telemetryFingerprintSource } from "../telemetryFingerprint.ts";

describe("telemetryFingerprintSource", () => {
  it("keeps one identity across a rebuild", () => {
    ...
    const before = telemetryFingerprintSource(
      "TypeError",
      "TypeError: x\n    at render (https://app/entry.iHryQ0pA.js:12:44)",
    );
    const after = telemetryFingerprintSource(
      "TypeError",
  ...
  it("separates different throw sites", () => {
    const a = telemetryFingerprintSource(
      "TypeError",
      "TypeError: x\n    at render (app.js:1:1)",
    );
    const b = telemetryFingerprintSource(
      "TypeError",
      "TypeError: x\n    at save (app.js:1:1)",
    );
  ...
  it("separates different error names at the same site", () => {
    ...
    expect(telemetryFingerprintSource("TypeError", stack)).not.toBe(
      telemetryFingerprintSource("RangeError", stack),
    );
  });
  ...
  it("uses the first frame, not the message line", () => {
    ...
    const withId = telemetryFingerprintSource(
      "Error",
      "Error: user 4711 not found\n    at load (app.js:3:9)",
    );
    const withOther = telemetryFingerprintSource(
      "Error",
  ...
  it("still fingerprints a stack with no frames", () => {
    ...
    const a = telemetryFingerprintSource("Error", "Script error.");
    const b = telemetryFingerprintSource("Error", "Out of memory");
  ...
  it("carries no app identifier, so a fingerprint survives moving sinks", () => {
    const fp = telemetryFingerprintSource(
      "Error",
      "Error: x\n    at f (app.js:1:1)",
    );
