 ...
 * the old `TelemetrySinkProvider.config`. It holds the secret `id`, so it is
 ...
 * from host code — e.g. `alepha.store.set(telemetryOptions, { excludedPaths: [...] })`
 ...
 * {@link telemetryClientAtom} for SSR delivery to the browser.
 */
export const telemetryOptions = $atom({
...
export type SigilOptions = Static<typeof telemetryOptions.schema>;
