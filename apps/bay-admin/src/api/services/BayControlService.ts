 ...
 * socket the authorization is the file mode, arbitrated by the kernel: bay-admin can
...
export class BayControlService {
  ...
  protected readonly env = $env(
    z.object({
      ...
      BAY_SOCKET: z.text({
        ...
        description:
          "Unix socket of the bay-go control API. Reachable only by members of the control group — bay-admin holds no token.",
      }),
    }),
  );
  ...
  async deploy(args: {
    ...
    // root-equivalent access is an operator act, and bay-admin has no per-user
