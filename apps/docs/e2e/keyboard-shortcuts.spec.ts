...
test.describe("Konami Code Easter Egg", () => {
  test("Konami code activates hacker mode", async ({ page }) => {
    await page.goto("/docs/introduction");
    await page.waitForLoadState("networkidle");
    ...
    // Check if hacker mode is activated (look for visual changes)
    const isHackerMode = await page.evaluate(() => {
      const html = document.documentElement;
      return (
        html.classList.contains("hacker") ||
        html.getAttribute("data-hacker") === "true" ||
        document.body.classList.contains("hacker")
      );
    });

    // Hacker mode may or may not be implemented, just verify no errors
