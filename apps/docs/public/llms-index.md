...
│   │   ├── Realm.ts      # $realm identity config (saas preset only)
...
Note: this layout is generated by `alepha init` and is identical in every Alepha project.
`controllers/` and `schemas/` come scaffolded; `services/`, `entities/`, `providers/` and
`shared/` are the conventional homes for what you add next. Each module
