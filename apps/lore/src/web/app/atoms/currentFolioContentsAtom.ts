...
/**
 * Pre-fetched contents of the current Folio directory — populated by
 * the `projectFolios` route loader so the page renders with data
 * already in hand (no useEffect-on-mount → no "Loading…" flash on
 * first navigation).
 *
 * `undefined` means "not loaded yet" (initial state) or "we're on a
 * non-folio route". Cleared on `onLeave`.
 *
...
export const currentFolioContentsAtom = $atom({
  name: "lore.folio.currentContents",
  description: "Pre-fetched Folio directory contents for the current route.",
  schema: z
    .object({
      directory: z
        .object({
