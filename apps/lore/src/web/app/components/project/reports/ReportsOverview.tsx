...
/**
 * Reports "Overview" page — a flat dashboard of project-wide quest health:
...
const ReportsOverview = (props: ReportsOverviewProps) => {
  ...
  const burnupChartConfig = {
    created: {
      label: tr("reports.overview.chart.created"),
      color: "var(--chart-3)",
    },
    completed: {
      label: tr("reports.overview.chart.completed"),
      color: "var(--chart-1)",
    },
  ...
  const completionChartConfig = {
    rate: {
      label: tr("reports.overview.chart.completionRate"),
      color: "var(--chart-2)",
    },
  } satisfies ChartConfig;
  ...
  const kpiRow: ReportsKpi[] = [
    ...
    {
      label: tr("reports.overview.kpi.activeMembers"),
  ...
  const attentionItems = [
    {
      ...
      label: tr("reports.overview.attention.stale"),
    },
    {
      count: attention.unassignedQuests,
      label: tr("reports.overview.attention.unassigned"),
    },
    {
      count: attention.blockedQuests,
      label: tr("reports.overview.attention.blocked"),
    },
  ].filter((item) => item.count > 0);

  return (
    <div className="flex flex-col gap-8">
      <ReportsKpiRow kpis={kpiRow} />

      <ReportsSection title={tr("reports.overview.burnup.title")}>
        {burnup.length > 0 ? (
          <ChartContainer
          ...
          <p className="text-muted-foreground py-6 text-center text-sm">
            {tr("reports.overview.empty")}
          </p>
        )}
      </ReportsSection>

      <ReportsSection title={tr("reports.overview.completionRate.title")}>
      ...
      {attentionItems.length === 0 ? (
        <p className="text-muted-foreground text-sm">
          {tr("reports.overview.attention.none")}
        </p>
      ) : (
