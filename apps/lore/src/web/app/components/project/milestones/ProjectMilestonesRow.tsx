...
export interface ProjectMilestonesRowProps {
 ...
 * left, with a milestone card to its right. Active milestone rows pulse;
 * closed rows are quiet. Click the card to open the detail sheet.
 */
const ProjectMilestonesRow = (props: ProjectMilestonesRowProps) => {
  const { tr } = useI18n<I18n, "en">();
  const i18n = useI18n();
  const dt = useInject(DateTimeProvider);
  const isActive = !props.milestone.closedAt;
  const tags = props.milestone.tags ?? [];
  ...
  return (
    <div className="relative flex gap-4">
      ...
      <div
        role="button"
        tabIndex={0}
        onClick={() => props.onOpenDetail(props.milestone)}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            props.onOpenDetail(props.milestone);
          }
        }}
        className={cn(
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            ...
            <div className="text-muted-foreground flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs">
              <span className="flex items-center gap-1">
                ...
                {tr("milestone.list.quests", {
                  args: [String(props.milestone.questCount)],
                })}
              </span>
              {props.milestone.closedAt && (
                <span>
                  {tr("milestone.list.closed", {
                    args: [
                      String(i18n.l(props.milestone.closedAt, { date: "ll" })),
                    ],
                  })}
                </span>
              )}
              {isActive && props.milestone.closesAt && (
                <span>
                  {tr("milestone.list.closesIn", {
                    args: [dt.of(props.milestone.closesAt).fromNow()],
                  })}
                </span>
              )}
              {isActive && !props.milestone.closesAt && (
                <span>
                  {tr("milestone.row.startedNow", {
                    args: [dt.of(props.milestone.createdAt).fromNow()],
                  })}
                </span>
              )}
            </div>
          ...
          {props.milestone.questCount === 0 && !isActive && (
            <Button
              variant="ghost"
              size="sm"
              ...
              onClick={(e) => {
                e.stopPropagation();
                props.onDelete(props.milestone.id);
              }}
              aria-label={tr("milestone.delete")}
            >
...
export default ProjectMilestonesRow;
