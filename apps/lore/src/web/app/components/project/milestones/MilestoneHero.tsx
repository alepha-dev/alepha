...
/**
 * Hero panel for the currently-active milestone. Renders like an open book
 * page: ornate left rail with the milestone number, large display-font
 * title, live stats, tag chips, and (when `closesAt` is set) a deadline
 * progress bar that fills as the milestone ages.
 */
const MilestoneHero = (props: MilestoneHeroProps) => {
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <div className="flex flex-col gap-1">
            <span className="text-muted-foreground text-xs uppercase tracking-[0.2em]">
              {tr("milestone.hero.eyebrow", {
                args: [String(milestone.number)],
              })}
            </span>
          ...
          <dl className="grid grid-cols-2 gap-4 pt-1 sm:grid-cols-4">
            <Stat
              ...
              label={tr("milestone.hero.stat.quests")}
              value={milestone.questCount}
            />
            <Stat
              icon={<BookOpenText className="size-4" />}
              label={tr("milestone.hero.stat.tags")}
              value={tags.length}
            />
            <Stat
              icon={<Hourglass className="size-4" />}
              label={tr("milestone.hero.stat.started")}
              value={dt.of(milestone.createdAt).fromNow(true)}
              text
            />
            {milestone.closesAt && (
              <Stat
                icon={<Hourglass className="size-4" />}
                label={tr("milestone.hero.stat.closesIn")}
                value={dt.of(milestone.closesAt).fromNow(true)}
                text
              />
            )}
          </dl>
