...
const OPEN_KEY = "lor.folio.activityPanel.open";
...
/**
 * Right-side "Recent activity" panel on the Folio browse page.
 * Project-scoped feed of folio revisions (create / edit / rename /
...
const FolioActivityPanel = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await folioApi.listProjectActivity({
        query: { projectId: project.id, limit: 50 },
      });
      setItems(res.items);
    } finally {
      setLoading(false);
          ...
          <ul className="flex flex-col">
            {(items ?? []).map((it) => (
              ...
              >
                ...
                <a
                  href={`/p/${project?.id}/folios/${it.folioShortId}`}
...
export default FolioActivityPanel;
