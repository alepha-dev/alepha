...
/** Mirror of CampaignBoardTable.getPriorityColor — kept local so the
...
const QuestView = (props: QuestViewProps) => {
  ...
  // Per-quest feature toggles live on the campaign. Undefined → off
  // (the new toggles default off for old campaigns until the owner
  // opts in via Settings → Quests).
  const questNoteEnabled = campaign?.features?.questNote === true;
  const questReminderEnabled = campaign?.features?.questReminder === true;
  const questChronoEnabled = campaign?.features?.questChrono === true;
      ...
      <div className="flex flex-1 flex-col overflow-x-hidden overflow-y-auto">
        <div className="flex flex-1 flex-col gap-6 px-5 py-4">
          ...
          <header className="bg-card border-border sticky top-0 z-10 -mx-5 -mt-4 flex items-center gap-3 border-b px-5 py-3">
            ...
            {((!quest.completedAt && campaign) || questChronoEnabled) && (
          ...
          {(questline.predecessor || questline.dependents.length > 0) && (
            <div className="flex flex-wrap items-center justify-center gap-1.5 text-xs">
              {questline.predecessor && (
                <Link
                  href={router.path("campaignQuestGraph", {
                    params: {
          ...
          {/* Notes — first-class quest field (was buried under Settings).
              Hidden when the per-campaign Quest Note toggle is off. */}
          {questNoteEnabled &&
            !quest.completedAt &&
