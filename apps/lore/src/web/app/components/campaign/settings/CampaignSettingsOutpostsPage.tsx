...
import CampaignSettingsOutpostRow from "./CampaignSettingsOutpostRow.tsx";
...
/**
 * Which machines report into this campaign.
 *
 ...
 */
const CampaignSettingsOutpostsPage = () => {
  ...
  const master = useCampaignFeatureToggle("outposts");
  ...
  const rotate = async (outpost: OutpostResource) => {
    ...
    try {
      const rotated = await outpostApi.rotateOutpost({
        params: { campaignId: campaign.id, outpostId: outpost.id },
      });
...
export default CampaignSettingsOutpostsPage;
