 ...
 * settings page next to the campaign edit form and the danger zone, sharing
 * their visual pattern (border, no shadow, multiple rows separated by
 * `divide-y` inside a single Card).
 */
const CampaignSettingsDataSection = () => {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      toaster.success(msg, {
        action: hasDetails
          ? {
              label: tr("campaign.settings.data.import.details"),
              onClick: () =>
                setDetails({
    ...
    <div className="flex flex-col gap-2">
        ...
        <CardContent className="flex flex-col gap-3 px-4 py-3 sm:grid sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:gap-6">
          ...
          <div className="flex justify-start sm:justify-end">
            ...
            >
              ...
              {tr("campaign.settings.data.import.choose")}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {tr("campaign.settings.data.import.preview.title")}
            </DialogTitle>
            <DialogDescription>
              {preview && preview.rowCount > 0
                ? tr("campaign.settings.data.import.preview.body", {
                    args: [String(preview.rowCount), preview.file.name],
                  })
                : tr("campaign.settings.data.import.preview.empty")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              ...
              disabled={importing}
            >
              {tr("campaign.settings.data.import.preview.cancel")}
            </Button>
            <Button
            ...
            >
              ...
              {tr("campaign.settings.data.import.preview.confirm")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export default CampaignSettingsDataSection;
