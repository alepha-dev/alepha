...
export class QuestTools {
  ...
  quest_create = $tool({
    ...
    handler: async ({ params }) => {
      ...
      // words; nothing here rewrites them.
  ...
  quest_complete = $tool({
    description:
      "Mark a quest as complete. Every objective must be either ticked or waived, and a quest filed under an epic completes only while that epic is 'ready' or 'in_progress' (the same rule as quest_accept). Completing (or shelving) the last open quest of an in-progress epic completes the epic. Pass `message` with a short summary of what was actually done — the summary is persisted on the quest, shown in the UI, and returned by `quest_get` / `project_context` so future agents working on this project can read it. Leaving it blank is allowed but wastes a free way to hand context to the next session. " +
  ...
  quest_delete = $tool({
    description:
      "Permanently delete a quest. Use this to clean up mistakenly-created quests. Only the quest creator or project owner can delete. Cannot be undone. Refused inside an 'in_progress' or 'completed' epic, whose quest set is frozen: a quest that will not be done there is shelved (quest_shelve), which keeps the record that it was specified and declined.",
    title: "Delete quest",
    annotations: {
      destructiveHint: true,
      idempotentHint: true,
