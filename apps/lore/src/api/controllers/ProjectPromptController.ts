...
export class ProjectPromptController {
   ...
   */
  //
  // ⚠️ Ported onto `requires` by epic #E39, which deleted `owner: true`.
  // Reading a prompt is `project:read`, editing one is `project:update` -
  // a prompt template is part of the project's own configuration, and the
  // permission is checked at application scope AND against the caller's rank
  // by the one option rather than by a `$secure` beside the gate.
  protected ownsAsMember = () =>
    $ownsProject({ param: "projectId", requires: "project:read" });
  protected ownsAsOwner = () =>
    $ownsProject({ param: "projectId", requires: "project:update" });

  getProjectPrompts = $action({
    use: [this.ownsAsMember()],
    method: "GET",
