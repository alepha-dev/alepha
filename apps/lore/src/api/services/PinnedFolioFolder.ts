...
/**
 * Folds pinned, non-protected folios into the `pinnedFolios` payload
 * surfaced by `project_context`.
 *
 * Its own service rather than a helper beside the MCP tool, so the cap
 * logic can be unit-tested — and substituted — without exercising the
 * MCP transport.
 */
export class PinnedFolioFolder {
  /**
   * Newest-updated folios come first (the caller is expected to pre-sort).
   * The total character budget across all included `content` is bounded by
   * `cap`:
   *
