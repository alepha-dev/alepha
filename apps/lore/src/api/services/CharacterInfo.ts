import { AlephaError } from "alepha";
import type { Quest } from "../entities/quests.ts";

export class CharacterInfo {
  // D&D-style 1–20 cap. Per-level deltas roughly track the existing ~14%
  // growth at the top end (247k → 282k → 322k). See folio
  // "Character — vision and economy".
  levels = [
    1080, 2200, 4800, 8400, 13000, 19000, 27000, 37000, 49000, 63000, 79000,
    97000, 117000, 139000, 163000, 189000, 217000, 247000, 282000, 322000,
  ];

  /**
   * Get rank of the quest based on its difficulty. It's not for user level.
   */
  getRank(n: number): string {
    if (n === 1) return "F";
    if (n === 2) return "C";
    if (n === 3) return "B";
    if (n === 4) return "A";
    if (n === 5) return "S";
    return "F";
  }

  getMoneyFromQuest(quest: Quest): number {
    const baseMoney = quest.difficulty * 40;
    const priorityBonus =
      quest.priority === "high" ? 200 : quest.priority === "medium" ? 100 : 0;
    return baseMoney + priorityBonus;
  }

  getGold(balance: number): number {
    return Math.floor(balance / 100);
  }

  getSilver(balance: number): number {
    return balance % 100;
  }

  getXpFromQuest(quest: Quest) {
    const priority =
      quest.priority === "high" ? 300 : quest.priority === "medium" ? 180 : 80;
    return quest.difficulty * 150 + priority;
  }

  getLevelByXp(xp: number): number {
    if (xp < 0) return 0;

    let acc = 0;
    for (let i = 0; i < this.levels.length; i++) {
      acc += this.levels[i];

      if (xp < acc) {
        return i + 1;
      }
    }

    return this.levels.length;
  }

  getNextXpForLevel(xp: number): number {
    return this.getGlobalMaxXpForLevel(this.getLevelByXp(xp)) - xp;
  }

  getMaxXpForLevel(level: number): number {
    const index = level - 1;
    if (index < 0 || index >= this.levels.length) {
      throw new AlephaError(`Invalid level: ${level}`);
    }
    return this.levels[index];
  }

  getGlobalMaxXpForLevel(level: number): number {
    const index = level - 1;
    if (index < 0 || index >= this.levels.length) {
      throw new AlephaError(`Invalid level: ${level}`);
    }
    let acc = 0;
    for (let i = 0; i <= index; i++) {
      acc += this.levels[i];
    }
    return acc;
  }

  getCurrentXpForLevel(level: number, xp: number): number {
    if (level === 1) {
      return xp;
    }
    return xp - this.getGlobalMaxXpForLevel(level - 1);
  }
}
