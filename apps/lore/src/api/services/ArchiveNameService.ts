...
import { archiveNames } from "../entities/archiveNames.ts";

/**
 * Sibling-name uniqueness enforcement for the Archive tree. Folios,
 ...
 *
 * Reservations live in `archive_names`. Each create/rename/move
 ...
 */
export type ArchiveNodeKind = "folio" | "blob" | "directory";

export interface ScopeKey {
  /** Directory UUID, or `undefined` when reserving at the campaign root. */
  parentDirectoryId?: string;
  /** Required when `parentDirectoryId` is undefined — `String(campaignId)`. */
  rootScope?: string;
}
...
export class ArchiveNameService {
  protected readonly names = $repository(archiveNames);
   ...
   * sentinel string (`root:<campaignId>` for root rows) so the index
  ...
  /**
   * Compute the non-NULL key written to `archive_names.parent_directory_id`.
