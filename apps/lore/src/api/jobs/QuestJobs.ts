...
export class QuestJobs {
   ...
   *
   * Runs every 5 minutes — finer than that wastes load for an email
   * cadence measured in days, coarser leaves people waiting if a
   * reminder hour just elapsed.
   *
   * Eligibility:
   * - `reminderNextAt <= now`
   ...
   * - `reminderIntervalMs IS NOT NULL`
   *
    ...
    handler: async () => {
      ...
      const due = await this.quests.findMany({
        where: {
          ...
          reminderIntervalMs: { isNotNull: true },
      ...
      for (const quest of due) {
        try {
          if (!quest.acceptedBy || !quest.reminderIntervalMs) continue;
          ...
          const nextAt = new Date(
            this.dt.nowMillis() + quest.reminderIntervalMs,
          ).toISOString();
          await this.quests.updateById(quest.id, {
