...
| `/:projectSlug/`                                             | `projectQuests`           | `project/ProjectQuestsPage.tsx`               | Quest list grouped by area. The project index, and always the landing of a bare `/:projectSlug` (see "Kanban is a route again" below)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `/:projectSlug/apps`                                         | `projectApps`             | `project/apps/ProjectApps.tsx`                | Every deployed copy of every app in one flat table - app, environment, address - in `AlephaTable` static-data mode over `currentInstancesAtom`, so no second request. It is the **Apps** sidebar entry's target: the per-app disclosure group that used to list instances was removed with epic #30, so this is the list. Gated on `features.sigils` like the instance page                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `/:projectSlug/settings/quality`                             | `projectSettingsQuality`  | `…/ProjectSettingsQualityPage.tsx`            | `features.quality` toggle: the only switch the Reports Quality tab has. The flag is absent from every project's defaults (a key there rebuilds the D1 `projects` table), so until this page existed a project could receive a run a day from CI and show none of them                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
**Apps is ONE sidebar entry, pointing at the list.** Between 2026-08-06 and epic #30 it was a collapsible group listing every enrolled app, which does not survive the model inversion: the leaves are now `(app, env)` pairs, so the group would either flatten a two-level thing into one level or nest a disclosure inside a disclosure for a project with three apps in four environments each. The entry is gated on `features.sigils` and always present under it - an empty project lands on the Apps page's own empty state, which is also where the create dialog lives, so there is nothing a hidden entry would save. The Blights sidebar entry follows the same data, plus one thing that outlives it: under the `features.sigils` master switch, it appears once some enrolled app's `kinds` carries `blights` **or** the project still has open blights. Both halves are load-bearing - `blights.sigilId` is `ON DELETE SET NULL` and rows are kept for `retentionDays ?? 30`, so dropping the capability (or deleting the last app) must not strand a triage queue that still exists behind a route that still resolves. The open-blight count is therefore fetched under `features.sigils` alone, not under the capability. The list reaches the page through `currentInstancesAtom`, filled by the `project` route loader (defensively: `listApps` is member-readable, but a failure costs the list, not the page).
