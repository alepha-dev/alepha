...
Kanban is not a route anymore — it's a second view of the `projectQuests` page (`ProjectQuestsPage.tsx` renders `KanbanBoard` instead of `ProjectQuestsTable`). The Header derives "on the board" from `currentProjectAtom` + `questsViewAtom` (`ProjectActionsCreateButton.tsx`) to:
