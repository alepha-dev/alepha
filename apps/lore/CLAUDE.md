...
| `/:projectSlug/apps`                                         | `projectApps`              | `project/apps/ProjectApps.tsx`                | Every deployed copy of every app in one flat table - app, environment, address - in `AlephaTable` static-data mode over `currentInstancesAtom`, so no second request. It is the **Apps** sidebar entry's target: the per-app disclosure group that used to list instances was removed with epic #30, so this is the list. Gated on the Apps capability like the instance page                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
`estateId` are the two that exist, both `ON DELETE SET NULL`. Environment
variables are #1813 and deploys are epic #1; each gets a column on
`app_instances` rather than a parallel table keyed on the pair. What the tab bar
