...
A project may name **one** of its open releases the **default** (epic #E48, `releases.defaultSince`): where a completed quest that names no release, and inherits none from its epic, lands, and what an epic begun without a release takes and carries down to its own release-less quests. ⚠️ **Not a third state.** A default release is still `open`, `ReleaseState` still has exactly two values, and the UI draws a second orthogonal chip beside the state one - folding it in would make the state filter lie. A fallback rather than a plan: zero defaults is normal, creating a release never picks one, publishing the default clears it (otherwise the next completion could not close), and reopening does not restore it. The swap is a single `UPDATE ... CASE ... RETURNING` in `DefaultReleaseService` with **no** partial unique index behind it - SQLite checks uniqueness per row as the update walks, so the guard would throw mid-swap. Written up in `docs/lore/1-guides/8-releases.md`.
...
| `/:projectSlug/apps`                                         | `projectApps`              | `project/apps/ProjectApps.tsx`                | Every deployed copy of every app in one flat table - app, environment, **version**, address - in `AlephaTable` static-data mode over `currentInstancesAtom`, so no second request. ⚠️ Version is the tag of that copy's newest SUCCEEDED deployment, filled by `AppController.toResources` in the same batched read as the sigil and the estate, and **never the newest artifact tag** - that is per app rather than per copy and is wrong on the first promotion. `latest` renders muted and never sorts as a semver. It is the **Apps** sidebar entry's target: the per-app disclosure group that used to list instances was removed with epic #30, so this is the list. Gated on the Apps capability like the instance page                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
