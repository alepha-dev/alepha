...
- Declarations: `src/api/services/CapabilityRegistry.ts` - one class, browser-safe, holding every capability's options, MCP tools, search kinds, activity kinds, dashboard cards and permission groups. The web reads it through the module-level instance in `src/web/app/services/capabilityRegistry.ts`.
...
| `/:projectSlug/quests`                                       | `projectQuests`            | `project/ProjectQuestsPage.tsx`               | Quest list grouped by area. Its loader gates on the **Work** capability and fetches nothing else; a bare `/:projectSlug` no longer reaches it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `/:projectSlug/reports/quality`                              | `reportsQuality`           | `project/reports/ReportsQuality.client.tsx`   | Coverage and test totals pushed by CI. The only Reports tab whose data is INGESTED rather than derived, which is why it is the only one carrying an empty state, a nothing-pushed-yet panel and a staleness line. The TAB is gated on the Apps capability AND on a run existing (`reportsTabs.ts`); the ROUTE is not, so a link someone already holds keeps working. ⚠️ Quality lost its own switch with epic #36 and joined the Apps baseline: it is pushed by CI under a CI credential and a switch must never turn somebody's build red, so `hasRun` is the honest question instead                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `/:projectSlug/apps`                                         | `projectApps`              | `project/apps/ProjectApps.tsx`                | Every deployed copy of every app in one flat table - app, environment, **version**, address - in `DataTable` static-data mode over `currentInstancesAtom`, so no second request. ⚠️ Version is the tag of that copy's newest SUCCEEDED deployment, filled by `AppController.toResources` in the same batched read as the sigil and the estate, and **never the newest artifact tag** - that is per app rather than per copy and is wrong on the first promotion. `latest` renders muted and never sorts as a semver. It is the **Apps** sidebar entry's target: the per-app disclosure group that used to list instances was removed with epic #30, so this is the list. Gated on the Apps capability like the instance page                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `/:projectSlug/apps/:app`                                    | `projectAppRedirect`       | `shared/RedirectPage.tsx`                     | An app name with no environment. Redirects to that app's default instance - `production` if it exists, else the first env by name (`schemas/defaultAppInstance.ts`, the same rule `AppService` uses) - so every pre-v3 `/apps/:appName` link still resolves in one invisible hop. A **sibling** of `projectApp`, not a parent: the router matches the two-segment route first, so a real instance URL never reaches this                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `/:projectSlug/apps/:app/:env`                               | `projectApp`               | `project/apps/AppLayout.tsx`                  | One deployed copy: the header, the tab bar and the loader every tab reads. ⚠️ **Two segments, never a joined slug** - hyphens are legal inside both halves, so `/apps/club-b14-production` is genuinely ambiguous between `club` + `b14-production` and `club-b14` + `production`, two rows that can coexist. ⚠️ The params are `:app` and `:env`, **never** `:id` or `:name` - see the router note below. The tab set is DATA (`appTabs.ts`), not a hand-written bar: a tab exists when the instance has unlocked it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `/:projectSlug/apps/:app/:env/`                              | `app`                      | `project/apps/AppDashboard.tsx`               | Identity (address, environment, sigil prefix, last report + silent badge past 24h) beside capabilities: what the app SAYS it sends (`reportedConfig`) next to what Lore ACCEPTS (`kinds`), so a disagreement is visible. An instance with no sigil gets the next-steps card instead of the identity block. **Issues no analytics query**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `/:projectSlug/apps/:app/:env/analytics`                     | `appAnalytics`             | `project/apps/AppAnalytics.client.tsx`        | One dense overview, Umami-shaped: chips + range/traffic controls, a metric row (visitors with a period delta, views, page loads, no-engagement share), the error budget, a full-width timeline, and six leaderboards in four tabbed cards each ending in a **More** link. Clicking a row filters the whole page. ⚠️ The traffic control is a segmented control and NOT a chip: it is a mode, and the only narrowing `uniqueVisitors` can honour. 404 when this instance has no sigil, or its `kinds` lacks `beacon`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `/:projectSlug/apps/:app/:env/analytics/:analyticsDimension` | `appAnalyticsDimension`    | `project/apps/AppAnalyticsDimension.tsx`      | One leaderboard in full, paged, where a card's **More** link goes. A SIBLING of `appAnalytics`, not a child. ⚠️ The segment is `:analyticsDimension` and not `:dimension`, for the reason `:app` is not `:id`. Validated against `ANALYTICS_DIMENSIONS` in the loader and 404s otherwise, because it is user input on its way to a query                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `/:projectSlug/apps/:app/:env/vitals`                        | `appVitals`                | `project/apps/AppVitals.tsx`                  | Web-vitals distributions and the per-page table. Same 404 rule as Analytics                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `/:projectSlug/apps/:app/:env/errors`                        | `appErrors`                | `project/apps/AppErrors.client.tsx`           | This instance's error groups, from `sigil_error_groups`. Unlocked by `blights` rather than `beacon` - the two kinds are genuinely independent, and an instance can carry either without the other                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `/:projectSlug/apps/:app/:env/explore`                       | `appExplore`               | `project/apps/AppExplore.tsx`                 | The admin analytics query builder pinned to this instance (`AdminAnalytics` with a per-app `transport`). Same 404 rule as Analytics. Registered in `projectViewRoutes.ts` like every other tab, which is what quest #1689 was about                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `/:projectSlug/apps/:app/:env/artifacts`                     | `appArtifacts`             | `project/apps/AppArtifacts.tsx`               | What CI has pushed for this app, one row per tag with its runtimes. Never gated: builds come from `lore artifacts push`, not from telemetry, so an instance with no sigil still has the tab. ⚠️ Artifacts belong to the **app**, so every instance of `club` shows the identical list and nothing on screen says why - carried knowingly, since on an instance page the list reads as "what can I deploy here"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `/:projectSlug/apps/:app/:env/deploy`                        | `appDeploy`                | `project/apps/AppDeploy.tsx`                  | What has been deployed here and what to ship next. Same gate as Environment: the `apps.deploy` option AND the copy having an estate. ⚠️ **No estate picker** - the estate is a property of the copy, chosen on its Settings tab and resolved server-side, and a picker here would be a second place to choose it. The build list is the Artifacts tab's `AppArtifactsList` with an `action` slot, never a second table. The log is read from Lore's own `deployments` rows, so it survives a Worker restart, and the poll switches off the moment nothing is `queued` or `running`. Rollback offers the fast Cloudflare `version_id` path and falls back to redeploying the stored artifact, with a confirmation naming how many migrations have landed since                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `/:projectSlug/apps/:app/:env/environment`                   | `appEnvironment`           | `project/apps/AppEnvironment.tsx`             | What this copy runs with. Gated on the `apps.deploy` option AND on the copy having an estate - the variables reach the app through a deploy, so a copy with nowhere to deploy has nothing to configure. ⚠️ **No response on this page's endpoints carries a value**, for anybody, the owner included: the list answers names, a four-character prefix of the long ones, and who set them last. Values are sealed at rest under `lore:app-secrets:v1` and opened only by the deploy. Route unguarded, like Artifacts: the tab self-hides and the endpoints refuse                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `/:projectSlug/apps/:app/:env/settings`                      | `appSettings`              | `project/apps/AppSettings.tsx`                | Five rows on `SettingsSection` / `SettingsRow` / `SettingsDangerSection`, one file each: rename the app half, rename the env half, the URL override, the sigil (mint / reveal once / rotate / delete), the estate, and delete the instance. ⚠️ **Each row owns its own PATCH and sends only its own key**, which is what lets five drafts share one instance without clobbering each other, and each is keyed by `instance.id` so moving between two instances' Settings tabs cannot save the value of the one you left. The four `kinds` switches and the feedback-button position were **deleted, not ported**. Mutations owner-only server-side                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `/:projectSlug/settings/apps`                                | `projectSettingsApps`      | `…/ProjectSettingsAppsPage.tsx`               | The **Apps** capability and its two options, `track` and `deploy`. ⚠️ `deploy` rendered disabled with a Soon badge from epic #36 until epic #1 shipped what it gates; nothing declares `soon` now, and the mechanism stays for the next surface announced before it exists. The sigil inventory lives here, read-only: minting happens on an instance's own Settings tab                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `/:projectSlug/settings/support`                             | `projectSettingsSupport`   | `…/ProjectSettingsSupportPage.tsx`            | The **Support** capability. No options                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `/account/estates`                                           | `accountEstates`           | `account/MyEstates.tsx`                       | The estates the signed-in user owns, across every project: create, rotate, the deploys and series switches and the stats interval, the projects each is lent to with detach, the command list with a `restart`, delete. `$pageAccount`, group `Lore`, order 103, gated on `listMyEstates.can()`. The admin backstop is `/admin/estates` (`LoreAdminRouter.adminEstates`, `AdminEstates.tsx`), list and delete only, reading no credential                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `/account/estates/:estateId`                                 | `bay`                      | `bay/BayLayout.tsx`                           | The console for one `bay` estate, a detail page inside the account shell (#E68): a header, route-linked tabs (`bayOverview` `/`, `bayApps` `/apps`, `bayApp` `/apps/:app/:env`, `bayCommands` `/commands`, `baySettings` `/settings`) and the tab under them, with Estates lit in the sidebar. Declared in `AppRouter` with `parent: this.account.layout`, every route `nav: { hidden: true }`. Owner-only: the loader's `getEstate` answers 404 for anyone else. It was `/bay/:estateId`, removed with no redirect; `bay` stays a reserved slug                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `/*`                                                         | `notFound`                 | `NotFound`                                    | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- `currentInstanceAtom` - the open deployed copy, `(app, env)` with its sigil and estate resolved. Every Settings row reads and rewrites it, which is why each is keyed by `instance.id`. Its analytics are **not** an atom any more: `currentSigilInsightsAtom` was deleted when the `projectApp` loader stopped fetching insights for every tab. Analytics, Vitals and the Dashboard each call `useAppInsights`, which keys a `useQuery` on `(sigil, range, traffic, dimension filters)` and reads them all from the URL
...
- Submit: any logged-in Lore user (no membership required), provided the project has the **Support** capability. Its switch on `/settings/support` is the owner's opt-in/out lever.
...
Epic #30 (2026-09-05) inverted the model: a project holds one row per **deployed
copy**, not one per credential. Folios #1185 and #1211 carry the design and the
review; what follows are the rules that get re-litigated without them.

**There is no `apps` table, and adding one is not the fix for anything here.** An
app is the distinct values of `app_instances.app` inside a project - a name that
exists because rows carry it and stops existing when the last one goes.
`AppController.listApps` answers `apps` (those distinct names) beside `items`, and
`app_instance_list` does the same over MCP, so the create dialog's combobox and an
agent both offer the existing names with no table to keep in step. The cost is real
and accepted: `clbu` beside `club` is silently a second app and nothing complains.
An entity would buy a rename that touches one row, and would cost a second write
path, a second uniqueness rule, and an empty-app state to design.

**An instance is the pair `(app, env)`**, unique per project
(`src/api/entities/appInstances.ts`). Both halves go through `APP_NAME_PATTERN`,
and the pair is bounded at 99 characters (`SIGIL_NAME_PAIR_MAX_LENGTH`) because
`sigils.name` mirrors it into a column that caps at 100.

**`env` is a free opaque slug.** There is no `environment` enum, no `tenant`
column and no `production` boolean, and none of the three is a missing feature:
`b14-production`, `eu-staging` and `pr-1841` are all legal, so a fleet slices as
finely as its operator wants. Two consequences before writing code against it.
Nothing may branch on `env === "production"`. And `defaultAppInstance`
(`src/api/schemas/defaultAppInstance.ts`) is the ONE place `production` means
anything - it picks that env if it exists, else the first by name - which is why
...
user-facing edits it and no screen shows it. `AppService.rename` writes the
instance and the mirror in the same method for exactly that reason, and
`updateSigil` lost its `name` field.

**The wire says `environment`, the column says `env`.** `@alepha/lore` has posted
`environment` on the sigil envelope since long before this epic, and that is an
external contract held by every deployed copy. The column is `env` because the
router, the UI and the MCP tools read it constantly and `app/env` is how the pair
is written everywhere else. Do not "unify" them: renaming the column changes
nothing outside Lore, and renaming the wire key breaks every deployed app in
silence.

**Each capability adds its own column here when its epic lands.** `sigilId` and
`estateId` are the two that exist, both `ON DELETE SET NULL`. ⚠️ Environment
variables turned out NOT to be a column: a copy has many of them, so #1813 gave
them a child table (`app_secrets`, cascading on `instanceId`), which makes
`app_instances` a cascade parent - read "Migration safety on D1" before any
later migration touches it. Deploys are epic #1 and did the same, with
`deployments`. What the tab bar
shows follows from those columns and never from a hand-written list - `appTabs.ts`
is data, and `unlockedBy` is a predicate over the instance.

**Creating an instance mints nothing.** It is a row saying a copy exists somewhere;
a sigil is one optional unlock on it, an estate is another. ⚠️ Deleting the sigil
destroys that instance's analytics history - all four aggregate tables cascade on
`sigilId` - while the instance itself survives; deleting the instance takes its
sigil with it. Rotate to revoke a leaked token, which keeps every row.

**Where to look**

- Entity: `src/api/entities/appInstances.ts` (its class doc opens with why there is no `apps` table)
- The one write path: `src/api/services/AppService.ts` - normalization, the pair bound, the name mirror, the estate check, and the orphan cleanup that deletes a fresh sigil when linking it to the instance fails (D1 has no transaction there, and an unlinked sigil is a live credential no page can reach)
- Controller: `src/api/controllers/AppController.ts` (`listApps` / `getApp` / `createApp` / `updateApp` / `deleteApp`; `toResources` does two batched reads and projects field by field, so `tokenHash` can never ride along)
- Default-instance rule: `src/api/schemas/defaultAppInstance.ts`
...
A **sigil** is the credential **one deployed copy** reports with. It hangs off an instance (`app_instances.sigilId`, `ON DELETE SET NULL`), and its `name` is a server-written mirror of `app/env` that no user edits - see "Apps and instances" above. It authenticates with a `sg_`-prefixed bearer token, stored hashed and shown once at creation; `tokenPrefix` exists so the UI can name a credential it cannot reconstruct. The `(projectId, name)` unique index survives v3 and is satisfied for free, since the pair it mirrors is already unique.
...
>
> Epic #30 (2026-09-05) restored the pair as a first-class row - `app_instances`, `(app, env)` - and inverted what owns what: the copy is the record, the credential is an unlock on it, and `sigils.name` became the derived mirror of a pair it used to encode by convention. Anything you read describing a sigil as "the app", or an enrol dialog on the settings page, predates that.

Minted from one instance's Settings tab at `/:projectSlug/apps/:app/:env/settings` and nowhere else - the project-level enrol dialog is gone, and `/:projectSlug/settings/sigils` is a read-only inventory beside the master toggle (`SigilController`, reads member-gated, mutations owner-gated - no role, no allowlist: owning the project is the whole gate). The **Apps** capability is the master switch, and its `track` option is what governs ingest. `blights` / `beacon` / `vitals` are **per-app**, the sigil's own `kinds` - set on that app's Settings tab, not a project-wide toggle. `feedback` is the one kind that also answers to a project-level switch (the **Support** capability, `/:projectSlug/settings/support`): the same flag governs the first-party form at `/:projectSlug/request`, which exists with no app enrolled at all, so it can't live only on a sigil. A newly enrolled sigil carries all four kinds by default; `SigilController.updateSigil` (`PATCH /projects/:projectId/sigils/:sigilId`, owner-only) is the write path, and it also **renames** an app: `name` is validated and its collision checked through the same `claimName` helper enrolment uses, so a name that could not be created is not reachable by renaming into it either. An omitted key means leave alone; only `url` treats `""` as clear, because a name has no such reading. ⚠️ A rename does NOT touch reporting - `SIGIL_KEY` is `sg_<projectSlug>_<secret>` and carries the project slug, not the app name - which is the opposite of the neighbouring rotate action and worth saying in the copy.
...
**Apps is ONE sidebar entry, pointing at the list.** Between 2026-08-06 and epic #30 it was a collapsible group listing every enrolled app, which does not survive the model inversion: the leaves are now `(app, env)` pairs, so the group would either flatten a two-level thing into one level or nest a disclosure inside a disclosure for a project with three apps in four environments each. The entry is gated on the Apps capability and always present under it - an empty project lands on the Apps page's own empty state, which is also where the create dialog lives, so there is nothing a hidden entry would save. The Blights sidebar entry follows the same data, plus one thing that outlives it: under the Apps capability, it appears once some enrolled app's `kinds` carries `blights` **or** the project still has open blights. Both halves are load-bearing - `blights.sigilId` is `ON DELETE SET NULL` and rows are kept for `retentionDays ?? 30`, so dropping the capability (or deleting the last app) must not strand a triage queue that still exists behind a route that still resolves. The open-blight count is therefore fetched under the Apps capability alone, not under the reporting kind. The list reaches the page through `currentInstancesAtom`, filled by the `project` route loader (defensively: `listApps` is member-readable, but a failure costs the list, not the page).
...
- UI - inventory: `src/web/app/components/project/settings/ProjectSettingsSigilsPage.tsx` (read-only list beside the master toggle; `ProjectSettingsSigilRow` and `ProjectSettingsSigilsEnrollDialog` were deleted with the enrol flow), `shared/TokenReveal.tsx`
- UI - per instance: `src/web/app/components/project/apps/AppLayout.tsx` (+ `AppDashboard`, `AppAnalytics`, `AppVitals`, `AppErrors`, `AppSettings` and its five `AppSettings*` rows, `useAppInsights` / `AppInsightsControls` - the per-tab fetch and its two toggles); the single Apps nav entry in `project/ProjectView.tsx`; atoms `currentInstancesAtom` / `currentInstanceAtom`
...
- `recharts` - reports charts. Every file that imports it is a `.client.tsx` (the six chart pages and `BayStatsChart`), which keeps it out of the server bundle; a new chart file must be one too, or recharts comes back to the Worker
...
- `settings-features.spec.ts` - the four capability settings pages and their switches
