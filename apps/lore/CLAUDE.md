...
**Controllers (26)** — `Artifact`, `Blight`, `Blob`, `Directory`, `Estate`, `Feedback`, `FeedbackComment`, `Folio`, `Insights`, `Invitation`, `Kanban`, `Quality`, `Release`, `Roadmap`, `Project`, `ProjectQuestPortability`, `ProjectReports`, `Quest`, `QuestComment`, `Sigil`, `SigilIngest`.
...
**Entities (32)** — `artifacts`, `blightIgnoreRules`, `blights`, `estates` (a user-owned deploy destination, lent to projects; epic #20, folio #1194), `feedback`, `feedbackComments`, `files`, `folioBlobs`, `folioDirectories`, `folioLinks`, `folioNames`, `folioRevisions`, `folios`, `identities`, `members`, `releases`, `projects`, `questComments`, `quests`, `sessions`, `sigilErrorGroups`, `sigilUniquesDaily`, `sigilViewsHourly`, `sigilVitalsHourly`, `sigils`, `users`.

**Services (41)** — `BlightRuleService`, `EstateService`, `EstateTokenService`, `FeedbackRateLimiter`, `FolioBlobService`, `FolioDirectoryService`, `FolioHistoryService`, `FolioLinkService`, `FolioNameService`, `PinnedFolioFolder`, `ProjectActivityService`, `ProjectLimits`, `ProjectSecurityService`, `QuestCsvFormatter`, `QuestCsvParser`, `QuestImportFormatProvider`, `QuestResourceMapper`, `QuestService`, `SigilIngestService`, `SigilTokenService`, plus `parsers/`.
...
| `/:projectSlug/apps`                                        | `projectApps`             | `project/apps/ProjectApps.tsx`                | Every enrolled app in one table (`AlephaTable` static-data mode over `currentSigilsAtom`, so no second request). **No sidebar entry on purpose** - the sidebar already carries a per-app disclosure group, so the breadcrumb's "Apps" segment is its only door (`SECTION_HREF_ROUTES` in `projectViewRoutes.ts`). Gated on `features.sigils` like the app page                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `/:projectSlug/apps/:appName/explore`                       | `appExplore`              | `project/apps/AppExplore.tsx`                 | The admin analytics query builder pinned to this app (`AdminAnalytics` with a per-app `transport`). 404 when this app's own `kinds` lacks `beacon`. Registered in `projectViewRoutes.ts` like every other tab, which is what quest #1689 was about                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `/:projectSlug/apps/:appName/artifacts`                     | `appArtifacts`            | `project/apps/AppArtifacts.tsx`               | What CI has pushed for this app, one row per tag with its runtimes (`AppArtifactsList`, the card that used to sit at the bottom of the Dashboard until feedback #2065). NOT gated on `beacon`: builds come from `lore artifacts push`, not from telemetry, so an app that collects nothing still has the tab. Reads the real registry through `ArtifactController.listArtifacts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
