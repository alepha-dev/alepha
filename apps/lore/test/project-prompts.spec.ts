...
describe("Project agent prompts", () => {
  ...
  it("refuses a member writing a template", async ({ expect }) => {
    ...
    await expect(
      ...
      // The gate, NAMED: a bare `.toThrow()` would also pass if the action
      // broke for an unrelated reason.
      //
      // ⚠️ It read `Only the project owner` until epic #E39, which replaced
      // `owner: true` with `requires: "project:update"`. The refusal names
      // the permission and the fix now, which is what an agent reading it can
      // act on - and "the owner" is no longer the whole answer, since an
      // Admin rank holds `project:update` too.
    ).rejects.toThrow(/does not grant project:update/);
  });
