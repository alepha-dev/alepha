...
describe("ProjectController reads through relations", () => {
  let ctx: TestContext;
  ...
  /**
   * The invariant the `members` and `projects` relations rest on.
   *
   * Both hop through `characters`. If a user could hold two characters in one
   * project, both relations would start returning duplicates — the project
   ...
   */
  it("allows only one membership per user per project", async ({ expect }) => {
  ...
  it("getProjectUsers returns every member exactly once", async ({
    expect,
  }) => {
    const owner = await createTestUser(ctx);
  ...
  it("getHomeOverview lists the user's projects, newest first", async ({
    ...
    const first = await ctx.projectController.createProject.fetch(
      { body: { title: "First" } },
      { user: owner },
    );
    const second = await ctx.projectController.createProject.fetch(
      { body: { title: "Second" } },
      { user: owner },
    );

    const response = await ctx.projectController.getHomeOverview.fetch(
      {},
      { user: owner },
    );
  ...
  /**
   * A member sees the project they joined, and only that one — the relation
   * hops through their own character, so someone else's project must not
   * come back with it.
   */
  it("getMyProjects shows a member their project and no other", async ({
    expect,
  }) => {
    const owner = await createTestUser(ctx);
    const member = await createTestUser(ctx);

    const joined = await ctx.projectController.createProject.fetch(
      { body: { title: "Joined" } },
      { user: owner },
    );
