...
describe("QuestController setQuestReminder", () => {
  ...
  it("enables a reminder: stores interval + schedules next trigger", async ({
    ...
    const after = await ctx.quests.setQuestReminder.fetch(
      { params: { id }, body: { intervalMs: 24 * 60 * 60 * 1000 } },
      { user },
    );

    expect(after.data.reminderIntervalMs).toBe(24 * 60 * 60 * 1000);
    const nextAt = new Date(after.data.reminderNextAt as string).getTime();
    // next trigger is interval ms in the future (allow drift for slow CI).
    expect(nextAt - before).toBeGreaterThanOrEqual(24 * 60 * 60 * 1000 - 1_000);
    expect(nextAt - before).toBeLessThanOrEqual(24 * 60 * 60 * 1000 + 5_000);
  });

  it("clears the reminder when intervalMs is null", async ({ expect }) => {
    const user = await createUser(ctx);
    const { id } = await seedAcceptedQuest(ctx, user);

    await ctx.quests.setQuestReminder.fetch(
      { params: { id }, body: { intervalMs: 60_000 } },
      { user },
    );
    const cleared = await ctx.quests.setQuestReminder.fetch(
      { params: { id }, body: { intervalMs: null } },
      { user },
    );
    expect(cleared.data.reminderIntervalMs).toBeUndefined();
  ...
  it("abandoning the quest clears any active reminder", async ({ expect }) => {
    ...
    expect(abandoned.data.reminderIntervalMs).toBeUndefined();
