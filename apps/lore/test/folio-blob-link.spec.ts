...
describe("rewriteFolioWikiLinks — assets/ references", () => {
  ...
  it("matches the attachment name case-insensitively", ({ expect }) => {
    ...
    // An empty alt is filled with the attachment's name, the same way the
    // `blob:` pass does — a rendered `<img>` with no alt text is an
    // accessibility hole, and the author wrote no better label to keep.
    const out = rewriteFolioWikiLinks(
      "![](assets/photo.jpg)",
      PROJECT_SLUG,
      [],
