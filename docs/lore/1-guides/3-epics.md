...
**Planned** is where an epic is specified. The quest set is being written:
...
started. A planned epic's quests stay out of the project's backlog, the
...
in and out. An epic can go back from ready to planned as long as nobody has
started it.
...
as ready** on a planned epic, **Back to planning** on a ready one. Mark as
...
- `epic_create` opens an epic in the planned status.
- `quest_create` with `epic_number` files quests into it, while it is
  planned or ready. With `accept: true` into a planned epic, the quest is
  created and the accept is refused as not ready, reported in the result
  rather than as an error.
- `epic_set_status` takes `ready` or `planned`, and nothing else. Deciding
  that a spec is done is the owner's call, so an agent refused because an
  epic is planned should say so rather than mark it ready itself.
- `quest_accept` and `quest_complete` work the quests. The first accept of a
  ready epic starts it; under a planned epic they answer that it is not
...
its status, or `includePlanned: true` to see everything.

Going the other way, an epic can hand ITSELF to an agent: **Review** asks
one to sharpen the plan while it is still open (planned or ready), and
...
a planned or ready epic reads as "not started" rather than "stalled". The
