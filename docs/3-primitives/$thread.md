...
|--------|------|----------|-------------|
| `name` | `string` | No | Unique name for this thread worker |
| `handler` | `Object` | Yes | The function to execute in the worker thread |
| `maxPoolSize` | `number` | No | Maximum number of worker threads in the pool |
| `idleTimeout` | `number` | No | Time in milliseconds before idle worker threads are terminated |
