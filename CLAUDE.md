...
- `yarn v --fast` - Inner-loop sanity check: lint + (typecheck, check:deps, check:conventions, check:docs, check:i18n, check:migrations) in parallel, then test + test:bun. Skips clean/copy/build/e2e and, like `yarn v`, all Go. Use for tight iteration **and as the gate before a commit**; reach for the full `yarn v` only when the change touches build/e2e territory. Note it now inherits the `--affected` and `--cache` defaults, so a `--fast` gate is a narrowed one: `--no-affected` when you want it whole.
- **Affected selection and caching are ON by default**, and `--no-affected` / `--no-cache` turn them off. Reach for `yarn v --no-affected --no-cache` when you want the exact, whole-repository run: before a release, or when a green result is surprising and you want to remove the two heuristics from the picture.
  - They default on because a speedup you have to remember to type is a speedup nobody gets, and because **CI does not run `yarn v` at all** — it runs `yarn lint`, `yarn typecheck`, `yarn alepha test --coverage`, `yarn e2e` and the rest as separate steps, so a local default cannot weaken it. `deploy-lore-production` has `needs: [verify, e2e]`, so a suite missed locally costs a red main and a blocked deploy, not a bad deploy.
- `--affected` restricts test, build and e2e to the workspaces the change can reach, from the workspace dependency graph. `--since <ref>` picks what it compares against (default `origin/main`). Measured on a one-file Lore change: 74s against 135s for `--fast`.
  - **A heuristic, and it says what it selected on every run.** It reasons about package boundaries, so it is blind to anything a package affects without declaring it: a fixture read by path, a service one suite leaves dirty for another.
  - Its unknowns resolve generously, which is the only reason it can be trusted at all: a repo-level file selects every workspace, an unplaceable path selects every workspace, a git that cannot answer degrades to verifying everything rather than to an empty change set, and an **empty diff selects every workspace rather than none**. A workspace with no vitest config contributes no `--project` filter, because an empty filter is not a filter that matches nothing, it is no filter at all.
  - ⚠️ **The empty-diff rule is the post-pull trap.** `--since` defaults to `origin/main`, so the moment you pull, `HEAD` **is** `origin/main` and the diff is empty by construction. Until 2026-09-04 that narrowed to zero workspaces and skipped test, build and e2e, so `yarn v` right after a pull exited 0 in ~70s having verified nothing at workspace level - a green that certifies a no-op. It now runs the pipeline whole instead and says so. To verify only what a pull brought in, name the pre-pull commit: `yarn v --since HEAD@{1}`.
...
- `--cache` skips steps that already passed against this exact tree, keyed on `HEAD^{tree}` plus a hash per dirty or untracked file. Measured: 187s for the first run, 3.2s for an identical second one, and a one-byte edit invalidates every step. The store is per user, under `$TMPDIR/alepha-cache-<uid>`, and shared between checkouts.
  - Not a heuristic like `--affected`: it skips only on a byte-identical tree, so the only way it misleads is a step that depends on something the tree does not describe.
...
| `3300-3399`                           | **dev servers** — `dev.port` in each app's `alepha.config.ts` (docs 3302, lore 3303, examples/playground 3304, examples/shop 3305, examples/errors 3306, examples/totp 3307, examples/ssr 3311), plus `@alepha/devtools`'s own Vite config (3310), which is not an Alepha app and has no `alepha.config.ts` |
